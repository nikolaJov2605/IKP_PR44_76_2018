			...
			for (int i = 0; i < lastIndex; i++)
			{
				...
				if (FD_ISSET(clientSockets[i], &readfds))
				{
					...
					if (iResult > 0)
					{
						...
						/*Queue* clientMessage = (Queue*)dataBuffer;
						...
						int size =atoi(token);							// od prvog elementa niza preuzima dimenziju matrice

						float* matrix_data = (float*)malloc(size * size);
						int idx = 0;
						...
						{
							printf("%f ,", matrix_data[i-1]);
							if (i % size == 0)
								printf("\n");
						}
						char convert[4];
						for (int i = 0; i < 4; i++) {
							convert[i] = dataBuffer[(size * size)*2 + 2 + i];
						}
						int socketInt = atoi(convert);
						SOCKET socketClient = socketInt;
